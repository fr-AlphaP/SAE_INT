...
FPORT = 2
HEX_ON = "04000100000000000064000000000000000000"

async def send_on():
    url = f"https://{TTN_CLUSTER_URL}/api/v3/as/applications/{APP_ID}/devices/{DEVICE_ID}/down/push"
    payload_b64 = base64.b64encode(bytes.fromhex(HEX_ON)).decode()
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if r.status_code == 200:
                print("✅ Effaroucheur allumé.")
            else:
                print(f"⚠️ Erreur d’allumage : {r.status_code} - {r.text}")
...
def main():
    try:
        ...
        if loop.is_running():
            asyncio.ensure_future(send_on())
        else:
            loop.run_until_complete(send_on())
    except RuntimeError:
        asyncio.run(send_on())  # fallback classique
